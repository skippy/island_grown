...
FunctionsFramework.http 'balance_check' do |request|
  ...
  return request.params['pretty'] ? JSON.pretty_generate(output) : output.to_json
...
def balance_info(card)
  ...
  Stripe::Issuing::Authorization.list(card: card.id).auto_paging_each do |a|
    auth = { approved: a.approved? }
    if auth[:approved]
      auth[:amount]         = (a.amount.to_f / 100).round(2)
      output[:total_spent] += auth[:amount]
    end
    auth[:merchant]         = a.merchant_data.to_hash.slice(:name, :city, :state, :postal_code)
    auth[:created_at]       = a.created
